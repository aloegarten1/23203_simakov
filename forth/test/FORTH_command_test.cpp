...
TEST(FORTH_COMMAND_WITH_LOOP, DO_LOOP_1)
{
    std::string res = repl(": pow over swap 1 do over * loop swap drop ; \n45 2 pow .");
    EXPECT_EQ(res, "2025");
}

TEST(FORTH_COMMAND_WITH_LOOP, DO_LOOP_ITER)
{
...
TEST(FORTH_COMMAND_WITH_IF, IF_BR1_FALSE)
{
    std::string res = repl(": ab if 87 emit then ;\n85 emit 0 ab 85 emit");
    EXPECT_EQ(res, "UU");
}

TEST(FORTH_COMMAND_WITH_IF, IF_BR1_TRUE)
{
    std::string res = repl(": ab if 87 emit then ;\n85 emit 1 ab 85 emit");
    EXPECT_EQ(res, "UWU");
}

TEST(FORTH_COMMAND_WITH_IF, IF_BR2_FALSE)
{
...
TEST(FORTH_COMMAND_WITH_IF, IF_BR2_TRUE)
{
    std::string res = repl(": ab if 65 emit then 66 emit else ;\n85 emit 1 ab 85 emit");
    EXPECT_EQ(res, "UAU");
}
